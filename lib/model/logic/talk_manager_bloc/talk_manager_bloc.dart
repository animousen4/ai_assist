...
class TalkManagerBloc extends Bloc<TalkManagerEvent, TalkManagerState> {
  ...
  TalkManagerBloc({required this.isTempl, required this.messageDatabase})
    ...
    chatSub = ((messageDatabase.select(messageDatabase.chats))
      ...
      add(_UpdateChats(await collectChats(
          await (messageDatabase.select(messageDatabase.messages)
                ..orderBy([
                  (u) =>
                      OrderingTerm(expression: u.data, mode: OrderingMode.desc)
                ])
                ..where((tbl) => tbl.messageStatus.equals(0)))
              .get())));
    }));
    ...
    on<AddChat>((event, emit) async {
      ...
      int id = await messageDatabase.into(messageDatabase.chats).insert(
          ChatsCompanion.insert(
              chatType: getChatType,
              chatName: event.chatName ?? "New chat",
              chatStatus: 0,
              isAutoGenerated: Value(event.isAutoGenerated),
              creationDate: DateTime.now()));

      emit(state.copyWith(toOpenChatId: id));
      //messageStream.publish()
    });

    on<AutoOpenChat>((event, emit) async {
      List<Chat> autoGenChats =
          await (messageDatabase.select(messageDatabase.chats)
                ..where((tbl) => tbl.isAutoGenerated.equals(true)))
              .get();
      for (var c in autoGenChats) {
        if ((await (messageDatabase.select(messageDatabase.messages)
                  ..where((tbl) => tbl.chatId.equals(c.chatId)))
                .get())
            .isEmpty) {
          emit(state.copyWith(
            toOpenChatId: c.chatId,
          ));
          return;
        }
      }
      add(AddChat(chatName: "New chat", isAutoGenerated: true));
    });
